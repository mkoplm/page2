...
public class MainView {
	...
	public static void main(String[] args) {
		MediaController music = new MediaController();
		CustomerController controll = new CustomerController();
		getPrint("이전 테이블 내용 삭제");
		music.deleteAll();
		controll.deleteAll();
		
		getPrint("테이블 추가");
		music.insert(new MediaDTO(1000,"야생화","박효신",1,500,19));
		music.insert(new MediaDTO(1001,"그런 남자","Bro",1,300,19));
		music.insert(new MediaDTO(1002,"썸(Feat.릴보이 Of 긱스)","소유,정기고",1,600,19));
...
//		music.insert(new MediaDTO(1009,"그중에 그대를 만나","이선희",1,300,19));
		...
		controll.insert(new CustomerDTO("custemer00","김세윤",24,7000,2000));
		controll.insert(new CustomerDTO("custemer01","김유리",24,9000,2001));
		controll.insert(new CustomerDTO("custemer02","김의태",28,2000,2002));
...
//		controll.insert(new CustomerDTO("custemer09","유병찬",27,1000,2009));
		...
		music.getMediaAll();
		System.out.println();
		controll.selectAllCustomer();
		
		getPrint("고객 정보 조회(custemer02)");
		controll.getCustomer("custemer02");
		
		getPrint("고객 정보 변경(custemer02) / 검색");
		controll.updateCustomer("custemer02", 300000);
		controll.getCustomer("custemer02");
		
		getPrint("음원 구매");
		controll.buyMedia("custemer02", 300, 1002);
